# Port Test: `CrossSection_HullError`

> **Read `prompts/tests/PREAMBLE.md` first.**

> **Test #43 of 58.** Port this single test to C. Make it pass. Commit. Then move to the next test.
...
make
TEST=CrossSection_HullError ./test_manifold          # run this test alone
...
3. Write the C test function `test_CrossSection_HullError(void)` in `src_c/test_manifold.c`.
4. Add `RUN_TEST(CrossSection_HullError);` to the appropriate section in `main()`.
5. Build and run: `TEST=CrossSection_HullError ./test_manifold`
...
8. Commit: `test: port CrossSection_HullError from C++`
...
1. This prompt is about ONE test: `CrossSection_HullError`. Do not work on other tests.
...
5. Commit: `test: port CrossSection_HullError from C++`
