# Port Test: `Boolean_MeshGLRoundTrip`

> **Read `prompts/tests/PREAMBLE.md` first.**

> **Test #1 of 58.** Port this single test to C. Make it pass. Commit. Then move to the next test.
...
make
TEST=Boolean_MeshGLRoundTrip ./test_manifold          # run this test alone
...
3. Write the C test function `test_Boolean_MeshGLRoundTrip(void)` in `src_c/test_manifold.c`.
4. Add `RUN_TEST(Boolean_MeshGLRoundTrip);` to the appropriate section in `main()`.
5. Build and run: `TEST=Boolean_MeshGLRoundTrip ./test_manifold`
...
8. Commit: `test: port Boolean_MeshGLRoundTrip from C++`
...
1. This prompt is about ONE test: `Boolean_MeshGLRoundTrip`. Do not work on other tests.
...
5. Commit: `test: port Boolean_MeshGLRoundTrip from C++`
