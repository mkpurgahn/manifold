# Port Test: `CrossSection_Rect`

> **Read `prompts/tests/PREAMBLE.md` first.**

> **Test #39 of 58.** Port this single test to C. Make it pass. Commit. Then move to the next test.
...
make
TEST=CrossSection_Rect ./test_manifold          # run this test alone
...
3. Write the C test function `test_CrossSection_Rect(void)` in `src_c/test_manifold.c`.
4. Add `RUN_TEST(CrossSection_Rect);` to the appropriate section in `main()`.
5. Build and run: `TEST=CrossSection_Rect ./test_manifold`
...
8. Commit: `test: port CrossSection_Rect from C++`
...
1. This prompt is about ONE test: `CrossSection_Rect`. Do not work on other tests.
...
5. Commit: `test: port CrossSection_Rect from C++`
