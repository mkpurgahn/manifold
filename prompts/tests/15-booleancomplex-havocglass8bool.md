# Port Test: `BooleanComplex_Havocglass8Bool`

> **Read `prompts/tests/PREAMBLE.md` first.**

> **Test #15 of 58.** Port this single test to C. Make it pass. Commit. Then move to the next test.
...
make
TEST=BooleanComplex_Havocglass8Bool ./test_manifold          # run this test alone
...
3. Write the C test function `test_BooleanComplex_Havocglass8Bool(void)` in `src_c/test_manifold.c`.
4. Add `RUN_TEST(BooleanComplex_Havocglass8Bool);` to the appropriate section in `main()`.
5. Build and run: `TEST=BooleanComplex_Havocglass8Bool ./test_manifold`
...
8. Commit: `test: port BooleanComplex_Havocglass8Bool from C++`
...
1. This prompt is about ONE test: `BooleanComplex_Havocglass8Bool`. Do not work on other tests.
...
5. Commit: `test: port BooleanComplex_Havocglass8Bool from C++`
