...
make
TEST=BooleanComplex_Sphere ./test_manifold          # run this test alone
...
3. Write the C test function `test_BooleanComplex_Sphere(void)` in `src_c/test_manifold.c`.
4. Add `RUN_TEST(BooleanComplex_Sphere);` to the appropriate section in `main()`.
5. Build and run: `TEST=BooleanComplex_Sphere ./test_manifold`
...
8. Commit: `test: port BooleanComplex_Sphere from C++`
...
1. This prompt is about ONE test: `BooleanComplex_Sphere`. Do not work on other tests.
