# Port Test: `Manifold_FaceIDRoundTrip`

> **Read `prompts/tests/PREAMBLE.md` first.**

> **Test #20 of 58.** Port this single test to C. Make it pass. Commit. Then move to the next test.
...
make
TEST=Manifold_FaceIDRoundTrip ./test_manifold          # run this test alone
...
3. Write the C test function `test_Manifold_FaceIDRoundTrip(void)` in `src_c/test_manifold.c`.
4. Add `RUN_TEST(Manifold_FaceIDRoundTrip);` to the appropriate section in `main()`.
5. Build and run: `TEST=Manifold_FaceIDRoundTrip ./test_manifold`
...
8. Commit: `test: port Manifold_FaceIDRoundTrip from C++`
...
1. This prompt is about ONE test: `Manifold_FaceIDRoundTrip`. Do not work on other tests.
...
5. Commit: `test: port Manifold_FaceIDRoundTrip from C++`
