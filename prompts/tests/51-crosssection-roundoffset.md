# Port Test: `CrossSection_RoundOffset`

> **Test #51 of 58.** Port this single test to C. Make it pass. Commit. Then move to the next test.
...
make
TEST=CrossSection_RoundOffset ./test_manifold          # run this test alone
...
3. Write the C test function `test_CrossSection_RoundOffset(void)` in `src_c/test_manifold.c`.
4. Add `RUN_TEST(CrossSection_RoundOffset);` to the appropriate section in `main()`.
5. Build and run: `TEST=CrossSection_RoundOffset ./test_manifold`
...
8. Commit: `test: port CrossSection_RoundOffset from C++`
...
1. This prompt is about ONE test: `CrossSection_RoundOffset`. Do not work on other tests.
...
5. Commit: `test: port CrossSection_RoundOffset from C++`
