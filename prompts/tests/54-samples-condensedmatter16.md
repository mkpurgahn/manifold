# Port Test: `Samples_CondensedMatter16`

> **Test #54 of 58.** Port this single test to C. Make it pass. Commit. Then move to the next test.
...
make
TEST=Samples_CondensedMatter16 ./test_manifold          # run this test alone
...
- **`CondensedMatter`** — find in `samples/src/condensed_matter.cpp`. If not yet ported to C, port it first.
...
3. Write the C test function `test_Samples_CondensedMatter16(void)` in `src_c/test_manifold.c`.
4. Add `RUN_TEST(Samples_CondensedMatter16);` to the appropriate section in `main()`.
5. Build and run: `TEST=Samples_CondensedMatter16 ./test_manifold`
...
8. Commit: `test: port Samples_CondensedMatter16 from C++`
...
1. This prompt is about ONE test: `Samples_CondensedMatter16`. Do not work on other tests.
...
5. Commit: `test: port Samples_CondensedMatter16 from C++`
