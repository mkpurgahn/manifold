# Port Test: `BooleanComplex_Ring`

> **Read `prompts/tests/PREAMBLE.md` first.**

> **Test #4 of 58.** Port this single test to C. Make it pass. Commit. Then move to the next test.
...
make
TEST=BooleanComplex_Ring ./test_manifold          # run this test alone
...
3. Write the C test function `test_BooleanComplex_Ring(void)` in `src_c/test_manifold.c`.
4. Add `RUN_TEST(BooleanComplex_Ring);` to the appropriate section in `main()`.
5. Build and run: `TEST=BooleanComplex_Ring ./test_manifold`
...
8. Commit: `test: port BooleanComplex_Ring from C++`
...
1. This prompt is about ONE test: `BooleanComplex_Ring`. Do not work on other tests.
...
5. Commit: `test: port BooleanComplex_Ring from C++`
