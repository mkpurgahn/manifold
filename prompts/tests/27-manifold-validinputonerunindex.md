# Port Test: `Manifold_ValidInputOneRunIndex`

> **Test #27 of 58.** Port this single test to C. Make it pass. Commit. Then move to the next test.
...
make
TEST=Manifold_ValidInputOneRunIndex ./test_manifold          # run this test alone
...
3. Write the C test function `test_Manifold_ValidInputOneRunIndex(void)` in `src_c/test_manifold.c`.
4. Add `RUN_TEST(Manifold_ValidInputOneRunIndex);` to the appropriate section in `main()`.
5. Build and run: `TEST=Manifold_ValidInputOneRunIndex ./test_manifold`
...
8. Commit: `test: port Manifold_ValidInputOneRunIndex from C++`
...
1. This prompt is about ONE test: `Manifold_ValidInputOneRunIndex`. Do not work on other tests.
...
5. Commit: `test: port Manifold_ValidInputOneRunIndex from C++`
