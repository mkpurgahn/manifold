# Port Test: `BooleanComplex_CraycloudBool`

> **Test #12 of 58.** Port this single test to C. Make it pass. Commit. Then move to the next test.
...
make
TEST=BooleanComplex_CraycloudBool ./test_manifold          # run this test alone
...
3. Write the C test function `test_BooleanComplex_CraycloudBool(void)` in `src_c/test_manifold.c`.
4. Add `RUN_TEST(BooleanComplex_CraycloudBool);` to the appropriate section in `main()`.
5. Build and run: `TEST=BooleanComplex_CraycloudBool ./test_manifold`
...
8. Commit: `test: port BooleanComplex_CraycloudBool from C++`
...
1. This prompt is about ONE test: `BooleanComplex_CraycloudBool`. Do not work on other tests.
...
5. Commit: `test: port BooleanComplex_CraycloudBool from C++`
