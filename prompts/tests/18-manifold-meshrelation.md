# Port Test: `Manifold_MeshRelation`

> **Test #18 of 58.** Port this single test to C. Make it pass. Commit. Then move to the next test.
...
make
TEST=Manifold_MeshRelation ./test_manifold          # run this test alone
...
3. Write the C test function `test_Manifold_MeshRelation(void)` in `src_c/test_manifold.c`.
4. Add `RUN_TEST(Manifold_MeshRelation);` to the appropriate section in `main()`.
5. Build and run: `TEST=Manifold_MeshRelation ./test_manifold`
...
8. Commit: `test: port Manifold_MeshRelation from C++`
...
1. This prompt is about ONE test: `Manifold_MeshRelation`. Do not work on other tests.
...
5. Commit: `test: port Manifold_MeshRelation from C++`
